...
export class Checker extends ParseTreeWalker {
    ...
    private _validateComparisonTypes(node: BinaryOperationNode) {
        ...
        if (isLiteralTypeOrUnion(rightType) && isLiteralTypeOrUnion(leftType)) {
            ...
            if (!this._evaluator.typesOverlap(leftType, rightType, assumeIsOperator, true)) {
        ...
        if (baseClassesWithPossiblyUncalledConstructors.length) {
            this._evaluator.addDiagnostic(
                ...
                LocMessage.multipleInheritance().format({
                    classes: baseClassesWithPossiblyUncalledConstructors.map((type) => type.shared.name).join(', '),
                }),
                errorNode
            );
        }
    ...
    private _validateSuperCallForMethod(node: FunctionNode, methodType: FunctionType, classType: ClassType) {
        ...
        if (this._fileInfo.diagnosticRuleSet.reportAbstractUsage !== 'none') {
            ...
            if (baseClass) {
                const baseMethodMember = lookUpClassMember(baseClass, methodType.shared.name, effectiveFlags);
                if (baseMethodMember) {
