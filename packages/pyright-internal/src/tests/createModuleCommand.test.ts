...
describe('CreateModuleCommand', () => {
    ...
    it('creates a module file in the target directory', async () => {
        await cmd.execute(makeArgs([root().toString(), root().toString(), 'my_module']), CancellationToken.None);
        expect(fs().existsSync(root().combinePaths('my_module.py'))).toBe(true);
    });

    it('strips .py extension from the input name', async () => {
        await cmd.execute(makeArgs([root().toString(), root().toString(), 'my_module.py']), CancellationToken.None);
        expect(fs().existsSync(root().combinePaths('my_module.py'))).toBe(true);
    });

    it('trims whitespace from the name', async () => {
        await cmd.execute(makeArgs([root().toString(), root().toString(), '  mymodule  ']), CancellationToken.None);
    ...
    it('falls back to parent directory when target is a file', async () => {
        ...
        await cmd.execute(makeArgs([root().toString(), fp.toString(), 'new_module']), CancellationToken.None);
    ...
    it('fails if target directory does not exist', async () => {
        await expect(
            cmd.execute(
                makeArgs([root().toString(), root().combinePaths('nope').toString(), 'm']),
    ...
    it('fails for name containing a dot', async () => {
        await expect(
            cmd.execute(makeArgs([root().toString(), root().toString(), 'my.module']), CancellationToken.None)
        ).rejects.toThrow();
    });
});
