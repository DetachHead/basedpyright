...
function hasForbiddenFileNameChars(name: string): boolean {
    for (const ch of name) {
        ...
        // NTFS/Windows forbidden characters
        if (ch === '<' || ch === '>' || ch === ':' || ch === '"' ||
            ch === '/' || ch === '\\' || ch === '|' || ch === '?' || ch === '*') {
            return true;
        }
...
export class CreateModuleCommand implements ServerCommand {
    ...
    async execute(cmdParams: ExecuteCommandParams, _token: CancellationToken): Promise<any> {
        ...
        if (fs.existsSync(moduleUri)) {
            this._ls.window.showErrorMessage(
                Service.moduleAlreadyExists().format({ name: moduleName })
            );
            return;
        }
        ...
        this._ls.window.showInformationMessage(
            Service.moduleCreated().format({ name: moduleName + '.py' })
        );
    }
}
...
export class CreatePackageCommand implements ServerCommand {
    ...
    async execute(cmdParams: ExecuteCommandParams, _token: CancellationToken): Promise<any> {
        ...
        if (fs.existsSync(packageDir)) {
            this._ls.window.showErrorMessage(
                Service.packageAlreadyExists().format({ name: packageName })
            );
            return;
        }
        ...
        this._ls.window.showInformationMessage(
            Service.packageCreated().format({ name: packageName })
        );
    }
}
