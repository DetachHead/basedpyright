...
export class CompletionProvider {
    ...
    protected checkIfDeprecated = (
        ...
        (!!primaryDecl &&
            !!this.evaluator.deprecatedTypingAlias(
                AnalyzerNodeInfo.getFileInfo(primaryDecl.node, this.nodeInfo),
                name,
                type,
    ...
    protected addSymbol(
        ...
        if (primaryDecl) {
            ...
            if (this.isEnumMember(detail.boundObjectOrClass, maybeEnumMember)) {
    ...
    private _addLiteralValuesForArgument(
        ...
        signatureInfo.signatures.forEach((signature) => {
            ...
            this.addValuesForTargetType(node, paramType, priorWord, priorText, postText, completionMap);
            return undefined;
        });
    }
    private _getEnumMemberCompletionsImpl(
        ...
        const scope = getScopeForNode(node, this.nodeInfo);
        ...
        this._getEnumMemberCompletionsImpl(
            ...
            (callback) =>
                ClassType.getSymbolTable(enumType).forEach((symbol, name) => {
                    if (this.isEnumMember(enumType, name)) {
    ...
    protected addValuesForTargetType(
        node: ParseNode,
        type: Type,
        priorWord: string,
        priorText: string,
            ...
            if (isLiteralType(v)) {
                if (isStr || isBytes) {
                    ...
                    if (typeof v.priv.literalValue === 'number' || typeof v.priv.literalValue === 'bigint') {
                        ...
                        this.addNameToCompletions(value, CompletionItemKind.Constant, priorWord, completionMap, {
                            sortText: this.makeSortText(SortCategory.LiteralValue, value),
                        });
        ...
        if (
            ...
            if (type) {
                ...
                if (containsCategoricalType) {
                    this.addValuesForTargetType(parseNode, type, priorWord, priorText, postText, completionMap);
                }

                // for StrEnum values
                const enumValueLiteralType = getStringLiteralValueTypeFromEnumType(this.evaluator, type);
                if (enumValueLiteralType) {
                    this.addValuesForTargetType(
                        parseNode,
    ...
    private _tryAddMatchCaseLiteralCompletions(
        ...
        if (type && this._containsCategoricalType(type)) {
            ...
            this.addValuesForTargetType(parentAndChild.child, type, priorWord, priorText, postText, completionMap);
            return true;
        }
    ...
    private _addLiteralValuesForExpectedTypes(
        ...
        for (const candidateType of expectedTypeResult.candidates) {
            if (!this._containsCategoricalType(candidateType)) {
                continue;
            }

            this.addValuesForTargetType(expressionNode, candidateType, priorWord, priorText, postText, completionMap);
